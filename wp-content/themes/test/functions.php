...
function sort_tag() {
    $sortNum = 0;
    $GLOBALS['postTags'] = [];
    $tmp_postTags = explode(",", $_POST['tax_input']['post_tag']);
    $postTags = [];
    foreach ( $tmp_postTags as $key => $value ) {
        $postTags[$key] = [];
        $postTags[$key]['name'] = $value;
        $postTags[$key]['sortKey'] = $sortNum;
        $sortNum++;
    }
    $GLOBALS['postTags'] = $postTags;
}
add_action('pre_get_posts', 'sort_tag'); //クエリが実行される前
add_action( 'save_post', function () {
    global $postTags;
    global $wpdb;
    $sortKey = 0;
    foreach ( $postTags as $postTag ) {
        $row = $wpdb->get_row("SELECT * FROM " . $wpdb->terms . " WHERE name = '". $postTag['name'] . "'", ARRAY_A);
        $wpdb->update( $wpdb->term_relationships, array( 'term_order' => $sortKey ),array( 'object_id' => get_the_ID(), 'term_taxonomy_id' => $row['term_id'] ));
        $sortKey++;
    }
});
